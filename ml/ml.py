...
def get_exam_prediction(exam_number, course_id, study_hours):
    ...
    for number in range(1, exam_number):
        exam = Exam.objects.filter(course_id=course_id, exam_number=number, is_hidden=False).first()
        score = (exam.final_score / 100) * 20
        previous_exams_score.append(score)
...
def get_hours_prediction(exam_number, course_id, predicted_score):
    ...
    exam_score = (model.predict([data])[0])/20 * 100
    return exam_score
